...
蒼狼 AI V2.2 現已搭載全新的智慧型啟動器，提供極簡的 Colab 使用體驗。

1.  **點擊 "Open in Colab" 徽章**:
    *   點擊本文件最上方的 [![在 Colab 中開啟](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/hsp1234-web/wolfAI_v1/blob/main/run_in_colab.ipynb) 徽章，即可在 Google Colaboratory 中打開最新的 `run_in_colab.ipynb` 筆記本。

2.  **選擇執行模式並執行儲存格**:
    *   筆記本中只有**一個程式碼儲存格**。
    *   在執行此儲存格之前，請先從上方的**下拉選單選擇您需要的執行模式**：
        *   **正常模式 (Normal Mode)**: 預設選項，專為一般使用者設計。此模式提供最穩定、簡潔的啟動流程。點擊執行後，系統會自動處理所有後台任務，最終目標是提供一個成功的服務連結，或是一個清晰、非技術性的中文錯誤引導。所有複雜的技術細節都將被隱藏。
        *   **除錯模式 (Debug Mode)**: 專為開發者設計。選擇此模式後，啟動過程將輸出詳細的安裝、執行日誌，並執行一系列自動化系統健康檢查（包括後端 API、前端服務、核心功能等）。這將生成一份詳盡的「系統體檢報告」，有助於快速定位潛在問題。
    *   選擇模式後，點擊該儲存格左側的 **▶️ 執行按鈕**。

3.  **等待服務啟動**:
    *   **正常模式下**: 您將看到進度指示。成功後，輸出區域會清除，並顯示一個可點擊的服務連結。如果遇到常見的端口衝突問題，會顯示圖文並茂的中文解決指引。
    *   **除錯模式下**: 您會看到詳細的執行日誌和健康檢查結果。成功後，同樣會提供服務連結。

4.  **設定 Colab Secrets (金鑰管理)**:
    *   為了應用程式功能的完整性（特別是 AI 分析和持久模式下的 Google Drive 操作），您需要在 Colab 環境的「密鑰 (Secrets)」管理器中設定以下金鑰。**建議在首次執行筆記本前完成設定**。
    ...
    *   **重要提示**:
        *   新的智慧型啟動器**不再區分暫存模式和持久模式的選擇**。要實現持久化儲存，您**必須正確設定上述 Google Drive 相關的四個 Colab Secrets**。如果這些 Secrets 未設定或不完整，系統將以類似「暫存模式」的方式運行，資料不會保存到您的 Google Drive。
        *   即使您不打算使用持久化儲存，也**強烈建議設定 `COLAB_GOOGLE_API_KEY`** 以體驗完整的 AI 功能。

### 開始使用

當 Colab 筆記本的程式碼儲存格成功執行完畢，並且顯示出前端訪問網址後，您就可以點擊該網址開始與您的蒼狼 AI 平台互動了。如果遇到任何問題，請仔細檢查 Colab 儲存格的輸出日誌（特別是在除錯模式下）。
