...
# Import app and the main module using absolute paths from backend
from app.main import app
import app.main as main_module # noqa: E402

client = TestClient(app) # app is already imported from app.main

TEST_BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__))) # This is /app/backend/tests
TEST_REPORTS_TEMP_DIR = os.path.join(TEST_BASE_DIR, "test_reports_data_temp") # /app/backend/tests/test_reports_data_temp
...
def test_chat_endpoint_mock_response():
    request_data = {
        "report_content": "這是關於A主題的週報。",
        "api_key": "testkey123",
        "selected_model": "gemini-mock-pro",
        "chat_history": [{"role": "user", "content": "你好"}],
        "user_question": "請總結這份週報的重點。"
    }
    response = client.post("/api/chat", json=request_data)
    assert response.status_code == 200
    json_response = response.json()
    assert "reply" in json_response
    assert "已收到您的提示。如果我是真實的" in json_response["reply"]
    assert request_data["selected_model"] in json_response["reply"]
    assert request_data["api_key"][:5] in json_response["reply"]
    # The prompt generated by prompt_service includes the user_question.
    # The mock AI reply in ai_service.py includes the first 100 chars of the prompt.
    # So we check for the presence of the question within that part of the reply.
    # Example of how user_question appears in prompt (from prompt_service.py):
    # **使用者最新問題:**
    # {user_question}
    # The (Note: ...) comment below was causing an indent error and has been re-aligned.
    # (Note: build_contextual_qa_prompt in prompt_service.py was updated)
    # The assertion below is removed because user_question is at the end of the prompt,
    # and prompt[:100] in the mock reply won't include it.
    # assert "請總結這份週報的重點。" in json_response["reply"]
    assert "這是一個模擬回覆" in json_response["reply"]
