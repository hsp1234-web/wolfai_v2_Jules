...
# logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s') # 舊的設定將被移除
# logger = logging.getLogger(__name__) # logger 將在 startup_event 中重新獲取

# --- 自訂台灣時區日誌格式化器 ---
class TaiwanTimeFormatter(logging.Formatter):
    """
    自訂日誌格式化器，將日誌記錄時間轉換並顯示為台灣時間 (Asia/Taipei)。
    """
    converter = lambda *args: datetime.now(pytz.timezone('Asia/Taipei')).timetuple()

    def formatTime(self, record, datefmt=None):
        """
        覆寫 formatTime 方法以使用 Asia/Taipei 時區。
        """
        ...
        if datefmt:
            ...
            except TypeError: # Python 3.5 or older
                s = dt.isoformat()
        return s

# 全局 logger 變數，將在 startup_event 中初始化
...
class HealthCheckResponse(BaseModel):
    status: str = "OK"
    message: str = "API 正常運行"
    scheduler_status: str = "未初始化"
    drive_service_status: str = "未初始化"
...
app = FastAPI(
    title="蒼狼 AI V2.2 後端服務",
    description="用於蒼狼 AI 可觀測性分析平台 V2.2 的後端 API 服務。提供數據處理、AI 分析排程、組態設定及狀態查詢等功能。",
    version="2.2.1" # 版本號更新
)
...
async def startup_event():
    global logger # 聲明我們要修改全局的 logger 變數

    # --- 設定日誌記錄器為台灣時區 ---
    log_formatter = TaiwanTimeFormatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
    log_handler = logging.StreamHandler()
    log_handler.setFormatter(log_formatter)

    root_logger = logging.getLogger() # 獲取根日誌記錄器
    if root_logger.hasHandlers():
        root_logger.handlers.clear() # 清除所有已存在的處理器
    root_logger.setLevel(logging.INFO) # 設定根日誌記錄器的級別
    root_logger.addHandler(log_handler) # 添加新的處理器

    logger = logging.getLogger(__name__) # 現在獲取 logger，它將繼承根記錄器的設定
    logger.info("後端應用程式啟動中 (時區: Asia/Taipei)...")
    ...
    if env_api_key:
        ...
        logger.info("AI 服務 API 金鑰 (COLAB_GOOGLE_API_KEY) 已從環境變數成功載入。")
    else:
    ...
    if service_account_json_content:
        try:
            ...
            logger.info("Google Drive 服務帳號憑證已從 GOOGLE_SERVICE_ACCOUNT_JSON_CONTENT 環境變數成功載入。")
        ...
        if os.path.exists(service_account_file_path):
            try:
                ...
                logger.info(f"Google Drive 服務帳號憑證已從檔案 {service_account_file_path} (GOOGLE_APPLICATION_CREDENTIALS) 成功載入。")
            except Exception as e:
                logger.error(f"從檔案 {service_account_file_path} 載入 Google Drive 服務帳號憑證失敗: {e}。")
                app_state["critical_config_missing_sa_credentials"] = True
        else:
    ...
    if app_state["critical_config_missing_sa_credentials"]:
        app_state["drive_service_status"] = "錯誤：Google 服務帳號憑證未設定或載入失敗"
    elif not app_state.get("service_account_info"):
    ...
    if operation_mode == "persistent":
        ...
        if app_state.get("report_ingestion_service") and not app_state["critical_config_missing_drive_folders"]:
            ...
            try:
                ...
                logger.info(f"APScheduler 排程器已啟動 (持久模式，時區: Asia/Taipei)，每隔 {scheduler_interval_minutes} 分鐘執行一次報告擷取任務。")
            except Exception as e:
...
async def health_check():
    ...
    if operation_mode == "persistent":
        if scheduler and scheduler.running:
            ...
            next_run_utc = job.next_run_time if job and job.next_run_time else None
            if next_run_utc:
                # 將 UTC 時間轉換為台北時間來顯示
                taipei_tz = pytz.timezone("Asia/Taipei")
                next_run_taipei = next_run_utc.astimezone(taipei_tz)
                next_run_str = next_run_taipei.strftime('%Y-%m-%d %H:%M:%S %Z')
            else:
                next_run_str = "未知"
            scheduler_status_msg = f"執行中 (模式: persistent, 時區: Asia/Taipei, 下次執行: {next_run_str})"
    ...
    if operation_mode == "persistent":
        if app_state.get("critical_config_missing_sa_credentials"):
            config_messages.append("錯誤：Google 服務帳號憑證未設定或載入失敗。")
...
async def set_api_key(payload: ApiKeyRequest):
    ...
    app_state["google_api_key_source"] = "使用者提供"
                    ...
                    try:
                        ...
                        if app_state.get("dal"):
                            ...
                            if not app_state["critical_config_missing_drive_folders"]:
                                scheduler_interval_minutes = int(get_env_or_default("SCHEDULER_INTERVAL_MINUTES", "15"))
                                # 修改 APScheduler 時區設定
                                new_scheduler = AsyncIOScheduler(timezone="Asia/Taipei")
                                ...
                                try:
                                    ...
                                    logger.info(f"APScheduler (使用者提供SA後, 持久模式, 時區: Asia/Taipei) 已啟動，每隔 {scheduler_interval_minutes} 分鐘執行。")
                        ...
                        app_state["drive_service"] = None
                        app_state["report_ingestion_service"] = None
                        app_state["drive_service_status"] = f"初始化錯誤 (使用者提供的SA)：{str(e_drive)[:100]} (持久模式)"
                        app_state["critical_config_missing_sa_credentials"] = True
            except json.JSONDecodeError:
                logger.info("使用者提供的 API 金鑰不是 JSON 格式。將其視為普通 AI API 金鑰，不影響 Drive 服務憑證 (持久模式)。")
        else:
            logger.info("持久模式下，Google Drive 服務帳號已從環境變數設定或先前已由使用者有效設定。此次提供的金鑰將僅用作 AI 服務 API 金鑰。")
    else:
        logger.info("暫存模式：使用者提供的 API 金鑰將被設定為 AI 服務 API 金鑰。不會用於初始化 Google Drive 相關服務。")
        app_state["drive_service_status"] = "暫存模式下未啟用"
        app_state["critical_config_missing_sa_credentials"] = False
...
if __name__ == "__main__":
    # --- 設定日誌記錄器為台灣時區 (針對本地開發) ---
    # 這段重複了 startup_event 中的設定，但確保直接執行 main.py 時日誌也使用台灣時區
    log_formatter_dev = TaiwanTimeFormatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
    log_handler_dev = logging.StreamHandler()
    log_handler_dev.setFormatter(log_formatter_dev)
    root_logger_dev = logging.getLogger()
    if root_logger_dev.hasHandlers():
        root_logger_dev.handlers.clear()
    root_logger_dev.setLevel(logging.INFO)
    root_logger_dev.addHandler(log_handler_dev)

    logger_dev = logging.getLogger(__name__) # 本地開發時使用的 logger 實例
    ...
    logger_dev.info("提示：若需測試排程器與 Google Drive 整合，請設定 WOLF_IN_FOLDER_ID, WOLF_PROCESSED_FOLDER_ID 及服務帳號憑證。")
    logger_dev.info("啟動 FastAPI 開發伺服器 (透過 __main__ 直接執行，時區: Asia/Taipei)...")
    port = int(os.getenv("PORT", 8000))
    host = os.getenv("HOST", "0.0.0.0")

    import uvicorn
    # uvicorn.run(app, host=host, port=port, log_level="info") # 預設的 uvicorn 日誌不會使用我們的 formatter
    # 若要 uvicorn 也使用自訂格式，需要更複雜的設定，此處保持簡單，主要讓應用程式日誌使用台灣時區
...
app.openapi_tags = [
    ...
    {
        "name": "設定",
        "description": "應用程式組態設定相關接口，例如 API 金鑰管理等。",
    },
]
